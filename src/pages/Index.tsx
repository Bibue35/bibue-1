    ...
    <>
      ...
      <section className="pt-24 sm:pt-28 pb-4">
        <div className="container mx-auto px-4">
          ...
          {/* Genre pills — single swipable row */}
          <div className="overflow-x-auto scrollbar-hide -mx-4 px-4 mt-2 touch-pan-x">
            <div className="flex items-center gap-1.5 w-max">
              {GENRE_PILLS.map((genre) => (
                <Link
                  key={genre}
                  to={`/genres/${genre.toLowerCase().replace(/[\s]+/g, "-")}`}
                  className="px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-medium text-muted-foreground hover:text-foreground bg-muted/40 hover:bg-foreground/5 transition-colors whitespace-nowrap"
                >
                  {genre}
                </Link>
              ))}
